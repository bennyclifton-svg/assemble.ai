...
function CardPlaceholder({ type, projectId }: { type: CardType; projectId: string }) {
  ...
  // Return actual DocumentCard if type is DOCUMENTS
  if (type === CardType.DOCUMENTS) {
    return (
      <DndProvider>
        <div className="h-full flex flex-col bg-white border-r border-gray-200 last:border-r-0">
          <DocumentCard projectId={projectId} />
        </div>
      </DndProvider>
    );
  }

  // Return actual ConsultantCard if type is CONSULTANT
  if (type === CardType.CONSULTANT) {
    return <ConsultantCard projectId={projectId} onClose={() => closeCard(type)} />;
  }

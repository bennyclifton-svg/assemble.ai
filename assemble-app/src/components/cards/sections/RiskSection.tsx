...
import { updateItemAction, getSectionItemsAction, initializeRiskSectionAction, generateRiskAction } from '@/app/actions/card';
...
export function RiskSection({ projectId }: RiskSectionProps) {
  ...
  useEffect(() => {
    const loadFields = async () => {
      ...
      const result = await getSectionItemsAction(projectId, 'risk');
      ...
      if (result.success && result.data.length > 0) {
        ...
        await initializeRiskSectionAction(projectId);

        // Reload fields
        const reloadResult = await getSectionItemsAction(projectId, 'risk');
  ...
  const handleGenerate = async () => {
    ...
    try {
      // Gather existing risks as context
      const existingRisks = fields
      ...
      const context = {
        ...
        risks: existingRisks || undefined,
        documentContent: documentContent || undefined,
      };

      const result = await generateRiskAction(projectId, context);

      if (result.success) {
        // Parse the AI response and update fields
        parseAndUpdateRisks(result.data);
      } else {
        alert(result.error?.message || 'Failed to generate risks');
      }
    } catch (error) {
      console.error('Error generating risks:', error);
      alert('Failed to generate risks');
    } finally {
      setIsGenerating(false);
    }
  };

  const parseAndUpdateRisks = (aiResponse: string) => {
    // Parse the AI response to extract the 4 risk categories
    const sections = ['Design', 'Construction', 'Financial', 'Schedule'];
    ...
    return (
      <div className="flex items-center justify-center h-48">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-600">Loading risks...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-gray-500">
        Tip: Fill in any existing risks, upload a project brief, then click "AI Generate" to identify risks with AI.
      </p>
    </div>
  );
}

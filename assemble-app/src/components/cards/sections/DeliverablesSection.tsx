...
import { generateDeliverablesAction, getDisciplineData, updateDeliverablesAction } from '@/app/actions/consultant';
...
export function DeliverablesSection({ projectId, disciplineId }: DeliverablesSectionProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (result.success) {
        setContent(result.data.deliverables || '');
      }
      setIsLoading(false);
    };
  ...
  useEffect(() => {
    if (content && content.trim() && !isLoading) {
      ...
      saveTimerRef.current = setTimeout(async () => {
        ...
        const result = await updateDeliverablesAction(projectId, disciplineId, content);
        if (result.success) {
          setSaveStatus('saved');
          setTimeout(() => setSaveStatus('idle'), 2000);
        } else {
          setSaveStatus('idle');
          console.error('Failed to save deliverables:', result.error.message);
        }
      }, 30000); // 30 seconds
    }
  ...
  useEffect(() => {
    return () => {
      ...
      if (currentContent && currentContent.trim()) {
        ...
        updateDeliverablesAction(projectId, disciplineId, currentContent);
      }
    };
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const context: ProjectContext = {
        ...
        existingDeliverables: content || undefined,
        documentContent: documentContent || undefined,
      };

      const result = await generateDeliverablesAction(context);
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <textarea
          ...
          placeholder="Define the deliverables here. Use bullet points to list drawings, reports, specs, etc. Click 'AI Generate' for AI-assisted content."
